...
async function testLabelCreationFixed() {
  try {
    console.log('🧪 Testing Label Creation (Fixed)...\n');
    ...
    if (rates.rateResponse?.rates && rates.rateResponse.rates.length > 0) {
      ...
      // Now try to create a label - FIXED: Just pass the rateId as a string
      console.log('\n🏷️ Creating label from rate...');
      
      try {
        // Method 1: Try with just rateId (correct way)
