...
async function testLabelCreationCorrect() {
  try {
    console.log('🧪 Testing Label Creation (Correct Method)...\n');
    ...
    if (rates.rateResponse?.rates && rates.rateResponse.rates.length > 0) {
      ...
      // Try the correct method: createLabelFromRate with just the rateId
      console.log('\n🏷️ Creating label from rate...');
      
      try {
        // The correct way: just pass the rateId as a string
