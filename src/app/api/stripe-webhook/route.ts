...
export async function POST(req: NextRequest) {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    // Send emails (no Shippo integration)
    try {
      ...
      if (customerEmail) {
        ...
        try {
          ...
          if (!customerEmailResult?.data?.id) {
            ...
            console.error('📋 Full customer email result:', customerEmailResult);
          }
      ...
      try {
        ...
        console.log('📧 Admin email details:', {
          ...
          subject: `🛍️ New Order Received! #${session.id}`
        });
        ...
        if (!adminEmailResult?.data?.id) {
          ...
          console.error('📋 Full admin email result:', adminEmailResult);
        }
