...
async function checkBeanieDetails() {
  console.log('=== Checking Beanie Product Details ===\n');
  
  try {
    const products = await stripe.products.list({ active: true, limit: 100 });
    const beanies = products.data.filter(p => p.name.toLowerCase().includes('beanie'));
    
    console.log(`Found ${beanies.length} beanie products:\n`);
    
    for (const product of beanies) {
    ...
    console.error('Error checking beanie details:', error.message);
  }
}
