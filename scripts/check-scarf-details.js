...
async function checkScarfDetails() {
  console.log('=== Checking Scarf Product Details ===\n');
  
  try {
    const products = await stripe.products.list({ active: true, limit: 100 });
    const scarves = products.data.filter(p => p.name.toLowerCase().includes('scarf'));
    
    console.log(`Found ${scarves.length} scarf products:\n`);
    
    for (const product of scarves) {
    ...
    console.error('Error checking scarf details:', error.message);
  }
}
